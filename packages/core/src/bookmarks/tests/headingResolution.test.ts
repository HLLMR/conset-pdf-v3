...
describe('Layout-Aware Heading Resolution Regression Tests', () => {
   ...
   */
  it('should resolve SECTION 23 00 00 to the page containing its actual heading', async () => {
   ...
   */
  it('should resolve SECTION 23 07 00 to actual heading page, not cross-reference', async () => {
    const inputPdf = findInputPdf();
    const docContext = new DocumentContext(inputPdf);
    await docContext.initialize();
    const pageCount = docContext.pageCount;
   ...
   */
  it('should not match cross-references in body text', async () => {
    const inputPdf = findInputPdf();
    const docContext = new DocumentContext(inputPdf);
    await docContext.initialize();
    const pageCount = docContext.pageCount;
